...
from .helper_modules.transformer_no_mlp_timm import Transformer
...
trf_3_stage_just_v_cfg = copy.copy(trf_3_stage_cfg)
trf_3_stage_just_v_cfg["just_v"] = True
...
default_cfgs = {
    'resnet34_s32_trf_frac_just_v_no_mlp_1': _cfg(
        url='',
        interpolation='bicubic'),
    'resnet34_s32_trf_frac_just_v_no_mlp_3': _cfg(
        url='',
        interpolation='bicubic'),

    'resnet34_s16_trf_frac_just_v_no_mlp_1': _cfg(
        url='',
        interpolation='bicubic'),
        
    'resnet34_s32_trf_frac_no_mlp_1': _cfg(
        url='',
        interpolation='bicubic'),        
    'resnet34_s32_trf_frac_no_mlp_3': _cfg(
        url='',
        interpolation='bicubic'),

    'resnet34_s16_trf_frac_no_mlp_1': _cfg(
        url='',
        interpolation='bicubic'),

    'resnet50_s32_trf_frac_just_v_no_mlp_1': _cfg(
        url='',
        interpolation='bicubic'),

    'resnet50_s16_trf_frac_just_v_no_mlp_1': _cfg(
        url='',
        interpolation='bicubic'),
        
    'resnet50_s32_trf_frac_no_mlp_1': _cfg(
        url='',
        interpolation='bicubic'),

    'resnet50_s16_trf_frac_no_mlp_1': _cfg(
        url='',
        interpolation='bicubic'),
}
...
class ResNetTrfFractal(nn.Module):
    ...
    def __init__(self, block, layers, trf_stage_cfg, num_classes=1000, in_chans=3,
        ...
        super(ResNetTrfFractal, self).__init__()
        ...
        # Fractal of transformers
        # TODO Still hardcoded to 256 x 256 images
        if output_stride == 32:
            self.stages = ["8", "4", "2"]
        elif output_stride == 16:
            self.stages = ["16", "8", "4", "2"]
        elif output_stride == 8:
            self.stages = ["32", "16", "8", "4", "2"]
        else:
    ...
    def reset_classifier(self, num_classes, global_pool='avg'):
        self.num_classes = num_classes
        _, self.fc = create_classifier(self.num_features, self.num_classes, pool_type=global_pool)
    ...
    def forward_fractals(self, x):
        # Embed input vectors to the appropriate dimension
        x = self.to_patch_embedding(x)

        step = 2
        transformers_intermediate_pred = tuple()
        # Go through every transformer level
        for stage in self.stages:
            # New input tensor dimensions
            b_dim, c_dim, h_dim, w_dim = x.shape

            # Rearange input
            # (B, C, H, W) --> (B H/2 W/2, 4, C)
            x = einops.rearrange(x, "b c (h1 h2) (w1 w2) -> (b h1 w1) (h2 w2) c", h2=step, w2=step)

            # Rearange positional encoding
            # (1, C, H, W) --> (B, C, H, W) --> (B H/2 W/2, 4, C)
            pos_encoding = self.pos_encodings[stage] 
            pos_encoding = einops.repeat(pos_encoding, "b1 c h w -> (b1 b2) c h w", b2=b_dim)
            pos_encoding = einops.rearrange(pos_encoding, "b c (h1 h2) (w1 w2) -> (b h1 w1) (h2 w2) c", h2=step, w2=step)
            ...
            x = einops.rearrange(x, "(b h1 w1) c -> b c h1 w1", h1=(h_dim//step), w1=(w_dim//step))

        return x.squeeze()

    def forward(self, x):
        x = self.forward_features(x)
        x = self.forward_fractals(x)
        if self.drop_rate:
...
def _create_resnet(variant, pretrained=False, **kwargs):
    return build_model_with_cfg(
        ResNetTrfFractal, variant, pretrained,
...
@register_model
def resnet34_s32_trf_frac_just_v_no_mlp_1(pretrained=False, **kwargs):
    ...
    return _create_resnet('resnet34_s32_trf_frac_just_v_no_mlp_1', pretrained, **model_args)

@register_model
def resnet34_s32_trf_frac_just_v_no_mlp_3(pretrained=False, **kwargs):
    kwargs["output_stride"] = 32

    trf_stage_cfg = trf_3_stage_just_v_cfg

    model_args = dict(block=BasicBlock, layers=[3, 4, 6, 3], trf_stage_cfg=trf_stage_cfg, **kwargs)
    return _create_resnet('resnet34_s32_trf_frac_just_v_no_mlp_3', pretrained, **model_args)


@register_model
def resnet34_s16_trf_frac_just_v_no_mlp_1(pretrained=False, **kwargs):
    kwargs["output_stride"] = 16

    trf_stage_cfg = trf_1_stage_just_v_cfg

    model_args = dict(block=BasicBlock, layers=[3, 4, 6, 3], trf_stage_cfg=trf_stage_cfg, **kwargs)
    return _create_resnet('resnet34_s16_trf_frac_just_v_no_mlp_1', pretrained, **model_args)

    
@register_model
def resnet34_s32_trf_frac_no_mlp_1(pretrained=False, **kwargs):
    kwargs["output_stride"] = 32

    trf_stage_cfg = trf_1_stage_cfg

    model_args = dict(block=BasicBlock, layers=[3, 4, 6, 3], trf_stage_cfg=trf_stage_cfg, **kwargs)
    return _create_resnet('resnet34_s32_trf_frac_no_mlp_1', pretrained, **model_args)   

@register_model
def resnet34_s32_trf_frac_no_mlp_3(pretrained=False, **kwargs):
    kwargs["output_stride"] = 32

    trf_stage_cfg = trf_3_stage_cfg

    model_args = dict(block=BasicBlock, layers=[3, 4, 6, 3], trf_stage_cfg=trf_stage_cfg, **kwargs)
    return _create_resnet('resnet34_s32_trf_frac_no_mlp_3', pretrained, **model_args)


@register_model
def resnet34_s16_trf_frac_no_mlp_1(pretrained=False, **kwargs):
    kwargs["output_stride"] = 16

    trf_stage_cfg = trf_1_stage_cfg

    model_args = dict(block=BasicBlock, layers=[3, 4, 6, 3], trf_stage_cfg=trf_stage_cfg, **kwargs)
    return _create_resnet('resnet34_s16_trf_frac_no_mlp_1', pretrained, **model_args)

# ...............................


@register_model
def resnet50_s32_trf_frac_just_v_no_mlp_1(pretrained=False, **kwargs):
    kwargs["output_stride"] = 32

    trf_stage_cfg = trf_1_stage_just_v_cfg

    model_args = dict(block=Bottleneck, layers=[3, 4, 6, 3], trf_stage_cfg=trf_stage_cfg,  **kwargs)
    return _create_resnet('resnet50_s32_trf_frac_just_v_no_mlp_1', pretrained, **model_args)


@register_model
def resnet50_s16_trf_frac_just_v_no_mlp_1(pretrained=False, **kwargs):
    kwargs["output_stride"] = 16

    trf_stage_cfg = trf_1_stage_just_v_cfg

    model_args = dict(block=Bottleneck, layers=[3, 4, 6, 3], trf_stage_cfg=trf_stage_cfg,  **kwargs)
    return _create_resnet('resnet50_s16_trf_frac_just_v_no_mlp_1', pretrained, **model_args)
@register_model
def resnet50_s32_trf_frac_no_mlp_1(pretrained=False, **kwargs):
    kwargs["output_stride"] = 32

    trf_stage_cfg = trf_1_stage_cfg

    model_args = dict(block=Bottleneck, layers=[3, 4, 6, 3], trf_stage_cfg=trf_stage_cfg,  **kwargs)
    return _create_resnet('resnet50_s32_trf_frac_no_mlp_1', pretrained, **model_args)


@register_model
def resnet50_s16_trf_frac_no_mlp_1(pretrained=False, **kwargs):
    kwargs["output_stride"] = 16

    trf_stage_cfg = trf_1_stage_cfg

    model_args = dict(block=Bottleneck, layers=[3, 4, 6, 3], trf_stage_cfg=trf_stage_cfg,  **kwargs)
    return _create_resnet('resnet50_s16_trf_frac_no_mlp_1', pretrained, **model_args)
